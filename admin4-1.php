<?php
  ...
  /*  หน้า input Class Management ห้องเรียน ปัจจุบันรับค่าจาก textbox ได้
      ห้องปกติ ความจุ 40 หรือ 80 ได้ */
?>
<!DOCTYPE html>
<html lang="en">
  <head>
  ...
  <body >
    <header>
      ...
      <form action="admin4-1.php" method="post">
        ...
        <!-- ปุ่มเด้งไปทางขวา -->
        <button name="save" style=" font-size: 40px;"class = "buttonsave">
...
<?php
  /*  font-end หน้านี้
     room code, size room ของห้องปกติ เก็บไว้ที่ตาราง table_room
    ...
    ***การ Refresh หน้า จะส่งข้อมูลเดิมซ้ำ ต้องเปลี่ยนหน้า ถึงจะ clear ข้อมูล*** */
        ...
        if(isset($_POST['save'])){
          ...
          while($row = $smyData->fetch_assoc()) {
              ...
              $room_type  = 'classroom';
              $room_size  = '40';
              $query = "INSERT INTO table_room(No,room_code,room_type,room_size)
                                    VALUES ('$count','$room_code','$room_type','$room_size')";
